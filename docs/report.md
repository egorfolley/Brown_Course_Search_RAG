...
- **Cold start:** Initial data pipeline (scrape + embed + index) takes 5-6 minutes
...
- **Async scraping:** Use `asyncio` + `aiohttp` for parallel scraping to reduce ETL time from 10min → ~3min
